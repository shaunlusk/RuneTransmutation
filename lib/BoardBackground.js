...
RuneTx.BoardBackground = function(screenContext, parentLayer, props) {
  ...
  this._graveOffsets = props.graveOffsets || {x:3,y:18};
  this._m = [];
  this._initialize();
};
...
RuneTx.BoardBackground.prototype._drawGraveyard = function() {
  ...
  for (i = 0; i < this._discardsAllowed; i++) {
    this._drawGrave(this._graveOffsets.x + i * 4, this._graveOffsets.y);
  }
  // draw dirt
  for (x = this._graveOffsets.x - 1; x < this._graveOffsets.x + 4 * this._discardsAllowed; x++) {
    this._parentLayer.drawSymbol("BLOCK", x, this._graveOffsets.y + 3, "#281900");
    this._parentLayer.drawSymbol("BLOCK", x, this._graveOffsets.y + 4, "#281900");
  }

  // draw border
  this._parentLayer.drawSymbol("ROUNDED_CORNER_TOP_LEFT", this._graveOffsets.x - 2, this._graveOffsets.y - 3, C64Style.Color.WHITE);
  this._parentLayer.drawSymbol("ROUNDED_CORNER_TOP_RIGHT", this._graveOffsets.x + 4 * this._discardsAllowed, this._graveOffsets.y - 3, C64Style.Color.WHITE);
  this._parentLayer.drawSymbol("ROUNDED_CORNER_BOTTOM_LEFT", this._graveOffsets.x - 2, this._graveOffsets.y + 5, C64Style.Color.WHITE);
  this._parentLayer.drawSymbol("ROUNDED_CORNER_BOTTOM_RIGHT", this._graveOffsets.x + 4 * this._discardsAllowed, this._graveOffsets.y + 5, C64Style.Color.WHITE);

  for (x = this._graveOffsets.x - 1; x < this._graveOffsets.x + 4 * this._discardsAllowed; x++) {
    this._parentLayer.drawSymbol("BAR_67", x, this._graveOffsets.y - 3, C64Style.Color.WHITE);
    this._parentLayer.drawSymbol("BAR_67", x, this._graveOffsets.y + 5, C64Style.Color.WHITE);
  }
  for (y = this._graveOffsets.y - 2; y < this._graveOffsets.y + 5; y++) {
    this._parentLayer.drawSymbol("|", this._graveOffsets.x - 2, y, C64Style.Color.WHITE);
    this._parentLayer.drawSymbol("|", this._graveOffsets.x + 4 * this._discardsAllowed, y, C64Style.Color.WHITE);
  }

  if (this._discardsAllowed > 1) {
    this._parentLayer.writeText("Graveyard", this._graveOffsets.x - 1, this._graveOffsets.y - 2, C64Style.Color.GREY);
  }
};
