...
RuneTx.GraveyardRenderer.prototype.disposeRune = function(runeTile, runeElement) {
  if (runeTile.equals(this._gameEngine.getDestructionRune())) {
    if (this._currentDiscards > 0) {
      this._runeTiles.pop();
      var element = this._runeElements.pop();
      this._foregroundLayer.removeElement(element);
...
RuneTx.GraveyardRenderer.prototype.restoreDiscardedRunes = function(runes) {
  this._currentDiscards = runes.length;
  runes.forEach(function(rune, i) {
    var runeTile = new RuneTx.RuneTile(rune._rune, rune._color);
    this._runeTiles.push(runeTile);
    var runeElement = RuneTx.createElementFromRune(
      runeTile,
      this._screenContext,
      this._foregroundLayer,
      {
        pixRenderer:this._pixRenderer,
        x:this._graveOffsets[i].runeX,
        y:this._graveOffsets[i].runeY
      }
    );
    this._runeElements.push(runeElement);
  }.bind(this));
...
RuneTx.GraveyardRenderer.prototype.getCurrentDiscards = function() {
  return this._currentDiscards;
};
