...
RuneTx.BoardForeground.prototype.restoreRune = function(serializedRuneTile, x, y) {
  var props = {
    x: this._boardOffsetX + x * this._cellWidth,
    y: this._boardOffsetY + y * this._cellHeight,
    pixPathArray : RuneTx.runes[serializedRuneTile._rune],
    pixRenderer : this._pixRenderer,
    defaultPalette : [serializedRuneTile._color]
  };
  var element = new C64Style.PixElement(this._screenContext, this._parentLayer, props);
  this._parentLayer.addElement(element);
  this._m[y][x] = element;
...
RuneTx.BoardForeground.prototype.removeRune = function(x, y) {
  this._parentLayer.removeElement(this._m[y][x]);
  this._m[y][x] = null;
};

RuneTx.BoardForeground.prototype.removeAllRunes = function() {
  for (var y = 0; y < this._rows; y++) {
    for (var x = 0; x < this._cols; x++) {
      if (this._m[y][x]) this._parentLayer.removeElement(this._m[y][x]);
    }
  }
};
