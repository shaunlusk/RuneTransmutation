...
RuneTx.LevelJumpManager = function(props) {
  ...
  this._levelXTarget = this._textOffsetX + ((this._text.length + 6) * C64Style.CELLWIDTH);

  this._jumpToLevelElement = new C64Style.TextElement(this._screenContext, this._layer, {
    text: this._text,
  ...
  this._layer.addElement(this._jumpToLevelElement);
...
RuneTx.LevelJumpManager.prototype.setLevel = function(level) {
  if (!this._isLevelValid(level)) return;
  this._level = level;
  this._render();
  this._saveSelectedLevel();
};

RuneTx.LevelJumpManager.prototype._isLevelValid = function(level) {
  if (this._level > this._levelMax) return false;
  if (this._level < this._levelMin) return false;
  return true;
};

RuneTx.LevelJumpManager.prototype._render = function() {
  var offset = 0;
  if (this._level >= 10) offset = -4;
  this._textElement.setX(this._levelXTarget + offset);
  this._textElement.setText(this._level.toString());
};

RuneTx.LevelJumpManager.prototype.getLevel = function() {return this._level;};

RuneTx.LevelJumpManager.prototype.loadSelectedLevel = function() {
  this.setLevel(RuneTx.GameStorage.getSelectedLevel());
};

RuneTx.LevelJumpManager.prototype._saveSelectedLevel = function() {
  RuneTx.GameStorage.updateSelectedLevel(this._level);
};
