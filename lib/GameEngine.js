...
RuneTx.GameEngine = function(screenContext, backgroundLayer, foregroundLayer, floatLayer, props) {
  ...
  this._cellWidth = 16 * screenContext.getScaleX();
  this._cellHeight = 16 * screenContext.getScaleY();
...
RuneTx.GameEngine.prototype._getBoardCoords = function(event) {
  return {
    // x : Math.floor((event.data.x - this._boardOffsetX) / (C64Style.CELLWIDTH * this._screenContext.getScaleX())),
    // y : Math.floor((event.data.y - this._boardOffsetY) / (C64Style.CELLHEIGHT * this._screenContext.getScaleY()))
    x: Math.floor((event.data.scaledX - this._boardOffsetX) / this._cellWidth),
    y: Math.floor((event.data.scaledY - this._boardOffsetY) / this._cellHeight),
  };
};
