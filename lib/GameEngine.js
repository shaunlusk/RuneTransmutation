...
RuneTx.GameEngine.prototype.handleMouseMove = function(event) {
  ...
  this._currentRuneElement.setX(event.data.x);
  this._currentRuneElement.setY(event.data.y);
};
...
RuneTx.GameEngine.prototype._getBoardCoords = function(event) {
  return {
    x : Math.floor((event.data.x - this._boardOffsetX) / this._currentLevelDefinition.cols),
    y : Math.floor((event.data.y - this._boardOffsetY) / this._currentLevelDefinition.rows)
  };
};
