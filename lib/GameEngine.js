...
RuneTx.GameEngine = function(props) {
  ...
  this._newGameButtonOffsetY = props.newGameButtonOffsetY || 26 * C64Style.CELLHEIGHT;
  this._cellWidth = 16;
  this._cellHeight = 16;
  ...
  this._newGameButton = new C64Style.TextButton(this._screenContext, this._textFloatLayer, {
    ...
    color: C64Style.Color.LIGHTGREEN,
    backgroundColor: C64Style.Color.BLACK,
    // x:this._screenContext.getUnScaledX(this._screenContext.getWidth()) - (4 * this._cellWidth),
    // y:this._screenContext.getUnScaledY(this._screenContext.getHeight()) - (3 * this._cellHeight),
...
RuneTx.GameEngine.prototype.handleUpdateHighScore = function(event) {
  RuneTx.GameSaver.updateHighScore(event.data.highScore);
};

RuneTx.GameEngine.prototype._loadGameState = function() {
  this._scoreRenderer.setHighScore(RuneTx.GameLoader.getHighScore());
};
