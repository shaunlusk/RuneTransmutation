...
RuneTx.ScoreRenderer = function(layer, offsetX, offsetY, highScoreOffsetX, highScoreOffsetY) {
  this._layer = layer;
...
RuneTx.ScoreRenderer.prototype.handlePlaceRune = function(event) {
  ...
  if (cell.changedLastIncrement()) {
    this._addToScore(RuneTx.Config.Score.Placement[cell.getDepth() - 1] * this._level);
  }
};

RuneTx.ScoreRenderer.prototype.handleLineClear = function(event) {
  var cell = event.data.cell;
  this._addToScore(RuneTx.Config.Score.LineClear * this._level);
};

RuneTx.ScoreRenderer.prototype.handleLevelClear = function(event) {
  var cell = event.data.cell;
  this._addToScore(RuneTx.Config.Score.LevelClear * this._level);
};
